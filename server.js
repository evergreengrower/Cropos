...
// ── Control switch Smartlife (bomba) ──────────────────────────────────────
app.post('/smartlife-switch', async (req, res) => {
  ...
  try {
    ...
    const body  = JSON.stringify({ commands: [{ code: 'switch_1', value: !!encender }] })
    ...
    const data = await r.json()
    if (data.success) {
      res.json({ ok: true, encendido: !!encender })
    } else {
      res.json({ ok: false, error: data.msg || 'Error Tuya', code: data.code })
    }
  } catch(err) {
